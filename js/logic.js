    ...
    for (let pack in game.cards) {
        ...
        for (let rarity in game.cards[pack]) {
            ...
            for (let id in game.cards[pack][rarity]) {
                ...
                // Stack effects additively: for each copy, increase effect by star and sum
                    let copies = (state.amount ?? 0) + 1;
                for (let effect of data.effects) {
                    let total = 0;
                    for (let i = 0; i < copies; ++i) {
                        // Optionally, you can make each copy stronger by increasing star per copy:
                        // total += effect(state.level, state.stars + i);
                        // But usually, just sum effect(level, stars) for each copy:
                        total += effect(state.level, state.stars);
                    }
                    list.push(total);
                }
