...
Math.floor = x => new Decimal(x).floor().toNumber();
Math.ceil = x => new Decimal(x).ceil().toNumber();
Math.round = x => new Decimal(x).round().toNumber();
Math.abs = x => new Decimal(x).abs().toNumber();
Math.max = (...args) => Decimal.max(...args.map(x => new Decimal(x))).toNumber();
Math.min = (...args) => Decimal.min(...args.map(x => new Decimal(x))).toNumber();
Math.pow = (x, y) => new Decimal(x).pow(y).toNumber();
Math.log = x => new Decimal(x).ln().toNumber();
Math.log10 = x => new Decimal(x).log10().toNumber();
// Note: Math.sin, cos, tan, etc. are not replaced here.
// This will affect all Math calls globally after util.js is loaded.
