...
function fixSave(game, newGame) {
    for (let thing in newGame) {
        if (!(thing in game)) {
            game[thing] = newGame[thing];
        } else if (typeof newGame[thing] == "object" && newGame[thing] !== null) {
            fixSave(game[thing], newGame[thing]);
        } else if (thing === "cardsDrawn" && (typeof game[thing] !== "number" || !isFinite(game[thing]))) {
            game[thing] = newGame[thing];
        }
    }
    // Special fix for stats.cardsDrawn
    if (game.stats && (typeof game.stats.cardsDrawn !== "number" || !isFinite(game.stats.cardsDrawn))) {
        game.stats.cardsDrawn = -Infinity;
    }
}