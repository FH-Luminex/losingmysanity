...
let tooltipTemplates = {
    ...
    card (pack, rarity, id, mode = null) {
        ...
        return (tooltip) => {
            ...
            let level = 1, stars = 1, amount = 0;
            if (state) ({level, stars, amount} = state);
            let curFx = [], newFx = null;
            fx = (x) => curFx[x];
            // Always count as having at least 1 card (base effect), plus additional copies
            let copies = (amount ?? 0) + 1;
            for (let f of data.effects) {
                let total = 0;
